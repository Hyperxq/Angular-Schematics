 ...
 */
class TreeWorkspaceHost implements WorkspaceHost {
 ...
 */
export async function readWorkspace(tree: Tree, path = DEFAULT_WORKSPACE_PATH): Promise<WorkspaceDefinition> {
    const host = new TreeWorkspaceHost(tree);

    const {workspace} = await workspaces.readWorkspace(path, host);
    if (!workspace) {
        throw new SchematicsException(`Don't find any workspace`);
    }
    return workspace;
}
